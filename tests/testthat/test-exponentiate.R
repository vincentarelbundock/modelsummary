...
test_that("exponentiate vector", {
    ...
    expect_equal(b, as.numeric(tab[["I"]][1:2]), ignore_attr = TRUE)
    expect_equal(b, as.numeric(tab[["II"]][1:2]), ignore_attr = TRUE)

    tab <- modelsummary(mod, exponentiate = TRUE,
                        output = "data.frame", statistic = NULL, fmt = identity)
    expect_equal(exp(b), as.numeric(tab[["I"]][1:2]), ignore_attr = TRUE)
    expect_equal(exp(b), as.numeric(tab[["II"]][1:2]), ignore_attr = TRUE)

    # standard error
    tab <- modelsummary(mod, exponentiate = TRUE, output = "data.frame",
                        estimate = "std.error", statistic = NULL, fmt = identity)
    expect_equal(exp(b) * se, as.numeric(tab[["I"]][1:2]), ignore_attr = TRUE)
    expect_equal(exp(b) * se, as.numeric(tab[["II"]][1:2]), ignore_attr = TRUE)
