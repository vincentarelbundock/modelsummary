...
In the [Appearance Vignette](https://modelsummary.com/articles/appearance.html) we saw how to customize the tables produced by the `modelsummary` function. The same customization possibilities are also available for all functions in the `datasummary_*` family of functions. Indeed, we can customize the tables produced by `datasummary` using the functions provided by `tinytable`, `gt`, `kableExtra`, `flextable`, `huxtable`, and `DT`. For instance, to customize tables using `tinytable`, we can do:
...
datasummary(All(penguins) ~ sex * (Mean + SD), data = penguins) |>
  style_tt(i = 3, background = 'cyan', color = 'red')
```
