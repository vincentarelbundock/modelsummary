...
By default, the `Percent()` function computes the share of each category with respect to the whole data, including missing observations, but we can work around this with some trickery:
...
datasummary(Factor(y) ~ N + Percent(), data = dat)
```
