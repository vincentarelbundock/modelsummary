...
# New information: `modelsummary_list`

`modelsummary_list` can also be used to add new columns of information that do not exist in the original model output. The example below shows how to combine the unstandardized estimates from `get_estimates()` with standardized betas computed by `parameters::standardize_parameters()`. We add the standardized values as a new `estimate_std` column, build a `modelsummary_list`, and then display both `b` and `beta` side by side using the `estimate` argument.
...
est <- get_estimates(mod)
std <- standardize_parameters(mod)
est$estimate_std <- std$Parameter[match(est$term, std$Parameter)]

gof <- get_gof(mod)

mod_list <- list(tidy = est, glance = gof)
class(mod_list) <- "modelsummary_list"

modelsummary(
  mod_list,
  estimate = c("b" = "{estimate}"),
  statistic = c("b (std)" = "{estimate_std}", "SE" = "std.error", "p" = "p.value"),
...
A `tidy_custom` method can also create a column which the default extraction did not produce at all. This is useful to label a model which has no grouped estimates when it is displayed alongside models which do. A `glm()` has no "response" column, so this method assigns one:
...
mod <- list(
  glm(am ~ mpg, data = dat, family = binomial),
  nnet::multinom(cyl ~ mpg, data = dat, trace = FALSE))
...
Without this method, the estimates of the `glm()` model would still be displayed, but in a single unlabelled column.
