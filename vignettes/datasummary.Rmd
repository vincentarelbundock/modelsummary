...
`datasummary` is a function from the `modelsummary` package which allows you to create data summaries, frequency tables, crosstabs, correlation tables, "Table 1s," and much more. It has many benefits:
...
* [The appearance of your tables can be customized](#customize) using the `gt`, `kableExtra` or `flextable` packages. The possibilities are endless.

This tutorial will show you how to use the `datasummary` function to create summary tables like these (and more!):
...
<center>
<img src="https://user-images.githubusercontent.com/987057/85772916-50f49f00-b6eb-11ea-8873-f09f097bbea7.png" width="49%">
...
training <- training %>%
            mutate(`Earnings pre-treatment` = re75 / 1000,
                   `Earnings post-treatment` = re78 / 1000,
                   Treatment = ifelse(treat == TRUE, 'Treatment', 'Control'),
                   Married = ifelse(married == TRUE, 'Yes', 'No')) %>%
