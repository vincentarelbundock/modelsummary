...
# `Arguments`: Missing values & Rounding
...
The shortcut functions that ship with `modelsummary` (e.g., `Mean`, `SD`, `Median`, `P25`) all use `na.rm=TRUE` by default. They also include an argument called `fmt` which can be used to format numbers (see `?sprintf`). We can thus round numbers by calling `Arguments`:
