...
`datasummary` is a function from the `modelsummary` package. It allows you to create data summaries, frequency tables, crosstabs, correlation tables, balance tables (aka "Table 1"), and more. It has many benefits:
...
In the next three sections, we illustrate how to use `datasummary_skim`, `datasummary_correlation`, and `datasummary_balance`. Then, we dive into `datasummary` itself to highlight its ease and flexibility.
...
A "balance table" or "Table 1" refers to a particular style of table which is often printed in the opening pages of a scientific peer reviewed article. Typically, this table will include basic descriptive statistics about different subsets of the study population. For instance, analysts may want to display the socio-demographic characteristics of members of the Control and Treatement groups in a randomized control trial, or the flipper length and male and female penguins. In addition, balance tables often include difference in means tests.
...
Now, we execute the `datasummary_balance` function. If the `estimatr` package is installed on your computer, `datasummary_balance` will calculate a difference in means.
...
Note that if your dataset includes columns called "blocks", "clusters", or "weights", these information will automatically be taken into consideration by `estimatr` when calculating the difference in means and the associated statistics.
...
If you do *not* want `datasummary` to format numbers, and you want to keep the numerical values instead of formatted strings, set `fmt=NULL`. This can be useful when post-processing tables with packages like `gt`, which allow you to transform cells based on their numerical content (this `gt` table will be omitted from PDF output):
...
# Case study: Balance table

```{r}
dat <- 'https://vincentarelbundock.github.io/Rdatasets/csv/Ecdat/Treatment.csv' %>%
        read.csv %>%
        mutate(`Earnings pre-treatment` = re75,
               `Earnings post-treatment` = re78,
               condition = ifelse(treat == TRUE, 'Treatment', 'Control'),
...
Now, we define a new function called `DiM`, which computes statistics using the `difference_in_means` function from the `estimatr` package. In this function, we assume that the dataset includes a variable named "condition". In addition, if the dataset includes columns called "clusters", "weights", or "blocks", `DiM` will calculate design-aware statistics:

```{r}
DiM <- function(x, data, statistic = 'estimate') {
    # make sure there is a column called "condition"
    if (!'condition' %in% colnames(data)) stop('To use the DiM function, your dataset must include a column named "condition".')
    ...
    # compute statistics
    out <- estimatr::difference_in_means(outcome_variable ~ condition, 
                                         data = data, 
                                         blocks = blocks,
                                         clusters = clusters, 
                                         weights = weights)
...
f <- All(dat) ~ 
     condition * (Mean + SD) + 
      (`Difference in Means` = DiM * Arguments(data = dat, statistic = 'estimate')) +
      (`Std.Error` = DiM * Arguments(data = dat, statistic = 'std.error')) +
      (`p` = DiM * Arguments(data = dat, statistic = 'p.value'))
