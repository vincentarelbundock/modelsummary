...
You can use clustered or robust uncertainty estimates by modifying the `vcov` parameter. This function accepts 5 different types of input.  You can use a string or list of strings:

```r
modelsummary(models, vcov = "robust")
modelsummary(models, vcov = list("classical", "robust", "stata", "HC4", "HC0"))
```
...
```r
library(sandwich)
modelsummary(models, vcov = vcovHC, statistic = 'p.value')
```

You can supply a list of functions of the same length as your model list:

```r
modelsummary(models, vcov = list(vcov, vcovHC, vcovHAC, vcovHC, vcov))
```
...
modelsummary(models, vcov = custom_stats)
```
...
# Custom coefficients and statistics
...
The next section shows that it is 
