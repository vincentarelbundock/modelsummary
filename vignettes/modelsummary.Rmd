...
If you provide a named character vector to `coef_rename`, only exact matches of the *complete* original term name will be replaced. If "drat" and "vs" appear as parts of the term name (e.g., "drat:mpg"), you need to pass a custom rename function to `coef_rename`:

```{r}
y <- list(lm(hp ~ drat / mpg, mtcars),
          lm(hp ~ vs / mpg, mtcars))
...
An important technical point is that `broom::tidy` still reports standard errors on the original scale, and not on the scale of the exponentiated coefficients. This is because, as [discussed on the `broom` issue tracker,](https://github.com/tidymodels/broom/issues/422) "quantiles (and thereby confidence intervals) are invariant under monotonic transformation (exp()) but variances are not."
