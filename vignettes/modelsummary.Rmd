...
This table is "OK", but the GLM intercept is very similar to the GAMLSS "sigma" intercept, so we may want to align those quantities. The problem is that `modelsummary` doesn't have the same statistical knowledge we do: Applying the `get_estimates` function to the GLM model does not produce a `parameter` column.
