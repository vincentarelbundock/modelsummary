...
Sometimes users want to include information more information than is made available by the default extractor function. To achieve this, we can define new `glance_custom` and `tidy_custom` methods. For example, the `polr` model from the `MASS` package does not produce p values, which means that we cannot use the `stars=TRUE` argument. Thankfully, we can use the `AER::coeftest` function to extract the p values we need to create stars.

To do this, we begin by estimating a model with the `MASS::polr`:
...
The `MASS::polr` produces an object of class `polr`:

```{r}
class(mod_ordinal)
```

To extract more (custom) information from a model of this class, we define a method called `tidy_custom.polr` which returns a data.frame with two columns: `term` and `p.value`:
