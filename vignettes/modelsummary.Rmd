...
The `shape` argument accepts a formula which determines the structure of the table, and can display "grouped" coefficients together (e.g., multivariate outcome or mixed-effects models).
...
```{r}
modelsummary(m, shape = term ~ model + statistic, gof_map = NA)
```

`shape` does partial matching, so the following command will produce the same result as above:

```{r, eval = FALSE}
modelsummary(m, shape = ~ statistic)
```

The order of terms in the formula determine the order of headers:
