...
Keep 3 digits, including at least two after the decimal (with optional scientific notation):

```{r}
modf <- lm(mpg ~ I(hp * 1000) + drat, data = mtcars)
...
An alternative mechanism to subset coefficients is to use the `coef_omit` argument. This string is a regular expression which will be fed to `grepl(x,perl=TRUE)` to detect the variable names which should be excluded from the table.
