...
If the vector is named, the values of this vector correspond to new variable labels as they will appear in the output table.

If the vector is not named, the output table will be sorted in the order in which terms are presented in `coef_map`. Coefficients which are *not* included in `coef_map` will be excluded from the table.
...
Models with and without grouped estimates can appear in the same table. When a model does not supply the group identifier---a `glm()` has no "response" column, for instance---its estimates are collected in a single unlabelled column:

```{r, warning = FALSE, message = FALSE}
dat_multinom$am <- factor(dat_multinom$am)

mod <- list(
    glm(am ~ mpg, data = dat_multinom, family = binomial),
    nnet::multinom(cyl ~ mpg, data = dat_multinom, trace = FALSE))

modelsummary(mod, shape = term ~ model + response)
```

To label that column instead of leaving it blank, we can define a `tidy_custom` method which adds the group identifier to the estimates of the ungrouped model. See the [Extension and customization vignette.](https://modelsummary.com/vignettes/modelsummary_extension.html)
