...
tab <- modelsummary(mod,
  ...
  metrics = "RMSE")
known <- c(
  "part", "response", "statistic", "(1) / (Intercept)", "(1) / mpg",
...
tab <- modelsummary(mod,
  ...
  metrics = "RMSE")
known <- c(
  "part", "response", "statistic", "(Intercept) / (1)", "(Intercept) / (2)",
  "mpg / (1)", "mpg / (2)", "drat / (2)")
expect_equivalent(colnames(tab), known)
expect_equivalent(dim(tab), c(6, 8))
...
expect_equivalent(
  colnames(tab),
  c(
    "part", "model", "statistic", "(Intercept) / 6",
...
expect_equivalent(
  colnames(tab),
  c(
    "part", "model", "statistic", "6 / (Intercept)", "6 / mpg", "6 / drat",
