...
- `align = "d"` with LaTeX output no longer mangles p-values printed
  with a leading comparator, such as `<0.001`. The comparator was
  typeset outside its column and collided with the neighboring one.
  Fixed upstream in `tinytable`. Issue \#880.
...
- Model packages are now attached on `future` workers, so
  goodness-of-fit rows are no longer silently dropped when a
  multi-worker `future` plan is active. Thanks to @raffaelemancuso for
  pull request \#971.
...
- New global options to label stub columns and improve accessibility in
  table headers. Fixes Issue \#930. Thanks to @jfieberg.
...
- In some locales, the HTML minus sign created problems in the output.
