...
#' @param group a two-sided formula with three components: "term", "model", and a
#' parameter group identifier (e.g., outcome levels of a multinomial logit model).
#' The group identifier must be the name of a column in the data.frame produced by
#' `get_estimates(model)`. The "term" component must be on the left-hand side of
#' the formula.
...
modelsummary <- function(
  ...
  for (i in 1:number_of_models) {
    ...
    # make sure there is a group column for merging
